            ...
            public void onClick(View v) {
                    ...
                    projeref.child(projekey).addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            String id ="";
                            for (DataSnapshot areaSnapshot : dataSnapshot.getChildren()) {
                                if (selected == areaSnapshot.child("Görev").getValue()) {
                                    id = areaSnapshot.getKey();
                                }
                            }
                            if(cb.isChecked()) {
                                projeref.child(projekey).child(id).child("Yapildi").setValue("Bitti");
                            }else if(!cb.isChecked()){
                                projeref.child(projekey).child(id).child("Yapildi").setValue("Henüz bitmedi");
                            }
                            Toast.makeText(Proje.this, "Seçilen Görev Kaydedildi", Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onCancelled(@NonNull DatabaseError databaseError) {
                            Toast.makeText(Proje.this,"",Toast.LENGTH_LONG).show();
                        }
            ...
            public void onClick(View v) {
                projeref.child(projekey).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        et6.setText(dataSnapshot.child(id).child("Yapildi").getValue().toString());
                    }
